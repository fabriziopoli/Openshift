<?php

class WerkstattController
{
	public function Werkstatt()
	{	
		require 'core/database.php';

        /*
            Aufträge filtern.
        */
		$filter = 'normal';
		if(isset($_GET['filter'])) {
			$filter = $_GET['filter'];
			if($filter == 'offene_auftraege') {
				$auftraegeSQL = $conn->query(
					'SELECT * FROM werkstatt WHERE delete_datum IS NULL AND status = "pendent" ORDER BY voraussichtliche_datum ASC, (CASE dringlichkeit
						WHEN "sehr hoch" THEN 1
						WHEN "hoch" THEN 2
						WHEN "normal" THEN 3
						WHEN "tief" THEN 4
						WHEN "sehr tief" THEN 5
					END) ASC;
				');
			} else if($filter == 'archive') {
				$auftraegeSQL = $conn->query('SELECT * FROM werkstatt WHERE delete_datum IS NOT NULL AND status = "abgeschlossen"');
			} else {
				$auftraegeSQL = $conn->query('SELECT * FROM werkstatt WHERE delete_datum IS NULL AND status = "pendent" ORDER BY id ASC');
			}
		} else {
			$auftraegeSQL = $conn->query('SELECT * FROM werkstatt WHERE delete_datum IS NULL AND status = "pendent" ORDER BY id ASC');
		}

		$auftraege = [];
		$animation_delay = 0;
        foreach($auftraegeSQL as $row) {
			$animation_delay = $animation_delay + 0.05;
            $id = $row['id'];
            $name = $row['kontaktperson'];
			$dringlichkeit = $row['dringlichkeit'];
			$status = $row['status'];
			$frist = $row['voraussichtliche_datum'];

			$functions = '';
			$fristCheck = '';
			if($filter != 'archive') {
				$functions = "<button onclick='getAuftrag(\"$id\")'><i class='fa-regular fa-pen-to-square'></i></button>
					<button onclick='delAuftrag(\"$id\")'><i class='fa-regular fa-trash-can'></i></button>";

				$aktuelleDatum = date_create(date('Y').'-'.date('m').'-'.date('d'));
				$fristDatum = date_create($frist);
				if($aktuelleDatum > $fristDatum) $fristCheck = '<i class="fas fa-thumbs-down" style="color:red;"></i>'; else $fristCheck = '<i class="fas fa-thumbs-up" style="color:green;"></i>';
			} 
			
            $auftraege[$id] = "
				<div class='auftrag' id='auftrag_$id' style='animation-delay:".$animation_delay."s;'>
					<p><span class='bez'>Name: </span><span class='name'>$name</span></p>
					<p><span class='bez'>Dringlichkeit: </span><span class='dringlichkeit'>$dringlichkeit</span></p>
					<p><span class='bez'>Status: </span><span class='status'>$status</span></p>
					<p><span class='bez'>Frist: </span><span class='frist'>$frist</span></p>
					$functions
					$fristCheck
					<input type='checkbox' name='mehrfachauswahl' class='hideContent' value='$id'/>
				</div>
			";
        }
		if(count($auftraege) < 1) $auftraege[0] = '<h2 class="keineErgebnisse"><i class="fas fa-exclamation-triangle"></i>Keine Aufträge gefunden</h2>
			<div id="messageBox" class="sendmessage">
				<div class="niokbutton" onclick="okbuttonsend()"></div>
			</div>
			<div id="main-frame-error" class="interstitial-wrapper">
				<div id="main-content">
					<img id="offline-resources-1x" style="display:none;" src="./public/assets/assets/default_100_percent/100-offline-sprite.png">
					<img id="offline-resources-2x" style="display:none;" src="./public/assets/assets/default_200_percent/200-offline-sprite.png">
					<div class="icon icon-offline" alt=""></div>
				</div>
				<div id="offline-resources">         
					<template id="audio-resources">
						<audio id="offline-sound-press" src="data:audio/mpeg;base64,"></audio>
						<audio id="offline-sound-hit" src="data:audio/mpeg;base64,"></audio>
						<audio id="offline-sound-reached" src="data:audio/mpeg;base64,"></audio>
					</template>
				</div>
			</div>
			<script src="public/js/dinoGame.js"></script>';
			
		$werkzeuge = [];
        foreach($conn->query('SELECT * FROM tools') as $row) {
            $id = $row['id'];
            $name = $row['name'];
            $werkzeuge[$id] = "<option value='$name'>$id</option>";
        }

		require 'app/Views/werkstatt.view.php';
	}
}

/*
    Erfassen eines Auftrags.
*/
if(isset($_POST['addAuftrag'])) {
    require '../../core/database.php';
    
    $fehler = [];
    if($_SERVER["REQUEST_METHOD"] === "POST") {
        $name = htmlspecialchars(trim($_POST["name"]) ?? "");
        $email = htmlspecialchars(trim($_POST["email"]) ?? "");
        $telefon = htmlspecialchars(trim($_POST["telefon"]) ?? "");
        $werkzeug = htmlspecialchars(trim($_POST["werkzeug"]) ?? "");
        $dringlichkeit = htmlspecialchars(trim($_POST["dringlichkeit"]) ?? "");
        $status = htmlspecialchars(trim($_POST["status"]) ?? "");

    	if($dringlichkeit == 'sehr tief') $voraussichtliche_datum = 'NOW() + INTERVAL 25 DAY'; elseif ($dringlichkeit == 'tief') $voraussichtliche_datum = 'NOW() + INTERVAL 20 DAY';
			elseif($dringlichkeit == 'hoch') $voraussichtliche_datum = 'NOW() + INTERVAL 10 DAY'; elseif ($dringlichkeit == 'sehr hoch') $voraussichtliche_datum = 'NOW() + INTERVAL 5 DAY'; 
			else { $voraussichtliche_datum = 'NOW() + INTERVAL 15 DAY'; $dringlichkeit = 'normal'; }

		$stmt = $conn->prepare("INSERT INTO werkstatt (kontaktperson, email, telefon, dringlichkeit, werkstatt.status, voraussichtliche_datum, werkzeug_fk) VALUES ('$name', '$email', '$telefon', '$dringlichkeit', '$status', $voraussichtliche_datum, (SELECT id FROM tools WHERE tools.name = '$werkzeug'))");

        //Validieren
        if($name === '') {
            array_push($fehler, 'Bitte geben Sie einen Namen ein.');
        }

        if($email === '') {
            array_push($fehler, 'Bitte geben Sie eine Email ein.');
        } elseif (strpos($email, '@') === false) {
            array_push($fehler, 'Die Email-Adresse ist ungültig.');
        }

        if($telefon !== '') {
            if(!preg_match('/^[\+ 0-9)]+$/', $telefon)) {
            array_push($fehler, 'Die Telefonnummer ist ungültig.');
            }
        }

        if($werkzeug === '') {
            array_push($fehler, 'Bitte wählen Sie ein Werkzeug aus.');
        }

        if($dringlichkeit === '') {
            array_push($fehler, 'Bitte wählen Sie die Dringlichkeit aus.');
        }
    }
	
    if($fehler != []) {
        echo json_encode(array('status'=>'error', 'fehler'=>$fehler));
    } else {
        $stmt -> execute();
        echo json_encode(array('status'=>'success', 'id'=>$name));
    }
}

/*
    Aufträge aus der Datenbank auslesen.
*/
if(isset($_POST['getAuftrag'])) {
    require '../../core/database.php';
    $fehler = [];
	$id = htmlspecialchars(trim($_POST["id"]) ?? "");
    foreach($conn->query("SELECT werkstatt.id AS AuftragID, kontaktperson, email, telefon, dringlichkeit, werkstatt.status, tools.name FROM werkstatt LEFT JOIN tools on tools.id = werkzeug_fk WHERE werkstatt.id = $id") as $row) {
		$id = $row['AuftragID'];
		$name = $row['kontaktperson'];
		$email = $row['email'];
		$telefon = $row['telefon'];
		$dringlichkeit = $row['dringlichkeit'];
		$status = $row['status'];
		$werkzeug = $row['name'];
	}

    if($fehler != []) {
        echo json_encode(array('status'=>'error', 'fehler'=>$fehler));
    } else {
        echo json_encode(array('status'=>'success', 'id'=>$id, 'name'=>$name, 'email'=>$email, 'telefon'=>$telefon, 'dringlichkeit'=>$dringlichkeit, 'aufStatus'=>$status, 'werkzeug'=>$werkzeug));
    }
}


/*
    Bearbeiten eines Auftrags.
*/
if(isset($_POST['editAuftrag'])) {
    require '../../core/database.php';

	$fehler = [];
    if($_SERVER["REQUEST_METHOD"] === "POST") {
		$stmt = $conn->prepare("UPDATE werkstatt SET kontaktperson = ?, email = ?, telefon = ?, werkzeug_fk = (SELECT id FROM tools WHERE tools.name = ?), werkstatt.status = ?, delete_datum = ? WHERE id = ?");
		$stmt -> bind_param("sssssss", $name, $email, $telefon, $werkzeug, $status, $delDate, $id);
        $name = htmlspecialchars(trim($_POST["name"]) ?? "");
        $email = htmlspecialchars(trim($_POST["email"]) ?? "");
        $telefon = htmlspecialchars(trim($_POST["telefon"]) ?? "");
        $werkzeug = htmlspecialchars(trim($_POST["werkzeug"]) ?? "");
        $status = htmlspecialchars(trim($_POST["status"]) ?? "");
		if($status != 'pendent' && $status != 'abgeschlossen') $status = 'pendent';
		if($status == 'abgeschlossen') $delDate = date('Y').'-'.date('m').'-'.date('d'); else $delDate = null;
        $id = htmlspecialchars(trim($_POST["id"]) ?? "");
		
        //Validieren
        if($name === '') {
            array_push($fehler, 'Bitte geben Sie einen Namen ein.');
        }

        if($email === '') {
            array_push($fehler, 'Bitte geben Sie eine Email ein.');
        } elseif (strpos($email, '@') === false) {
            array_push($fehler, 'Die Email-Adresse ist ungültig.');
        }

        if($telefon !== '') {
            if(!preg_match('/^[\+ 0-9)]+$/', $telefon)) {
            array_push($fehler, 'Die Telefonnummer ist ungültig.');
            }
        }

        if($werkzeug === '') {
            array_push($fehler, 'Bitte wählen Sie ein Werkzeug aus.');
        }

        if($status === '') {
            array_push($fehler, 'Bitte wählen Sie die Status aus.');
        }
    }
	
    if($fehler != []) {
        echo json_encode(array('status'=>'error', 'fehler'=>$fehler));
    } else {
		$stmt -> execute();
        echo json_encode(array('status'=>'success', 'id'=>$id, 'name'=>$name, 'aufStatus'=>$status));
    }
}

/*
    Löschen eines Auftrags.
*/
if(isset($_POST['delAuftrag'])) {
    require '../../core/database.php';
    $fehler = [];
    if($_SERVER["REQUEST_METHOD"] === "POST") {
        $stmt = $conn->prepare("UPDATE werkstatt SET delete_datum = NOW(), status = 'abgeschlossen' WHERE id = ?");
        $stmt -> bind_param("s", $id);
        $id = htmlspecialchars(trim($_POST["id"]) ?? "");
        $stmt -> execute();
    }

    if($fehler != []) {
        echo json_encode(array('status'=>'error', 'fehler'=>$fehler));
    } else {
        echo json_encode(array('status'=>'success', 'id'=>$id));
    }
}


/*
    Status eines Auftrags ändern.
*/
if(isset($_POST['editAuftragStatus'])) {
    require '../../core/database.php';
    $fehler = [];
    if($_SERVER["REQUEST_METHOD"] === "POST") {
        $stmt = $conn->prepare("UPDATE werkstatt SET delete_datum = NOW(), status = 'abgeschlossen' WHERE id = ?");
        $stmt -> bind_param("s", $id);
        $id = htmlspecialchars(trim($_POST["id"]) ?? "");
        $stmt -> execute();
    }

    if($fehler != []) {
        echo json_encode(array('status'=>'error', 'fehler'=>$fehler));
    } else {
        echo json_encode(array('status'=>'success', 'id'=>$id));
    }
}